...
def timeline_html(months, growth, amazon, money):
    ...
    for title, values, color in [('Dein Kontozuwachs pro Monat', growth, '#159c62'), ('Amazon-Ausgaben pro Monat', amazon, '#5271ee')]:
        ...
        for month, value in zip(months, values):
            ...
            cells.append(f'<div class="month-cell"><div style="height:200px;position:relative" role="img" aria-label="{escape(month)}: {label}"><div style="position:absolute;left:0;right:0;top:{baseline}px;border-top:1px solid #cbd5e1"></div><div style="position:absolute;left:25%;width:50%;top:{top}px;height:{height}px;border-radius:5px;background:{color if value >= 0 else "#d65754"}"></div><div class="month-value" style="top:{max(0, top-27)}px">{label}</div></div><span>{escape(month)}</span></div>')
    ...
    .timeline-panel h3{margin:0 0 12px;font-size:24px}.month-scroll{overflow-x:auto;overflow-y:hidden;clip-path:inset(0);cursor:grab;touch-action:pan-x pan-y;scrollbar-width:auto}
    .month-scroll:active{cursor:grabbing}.month-grid{display:grid;width:100%;user-select:none}.month-cell{overflow:hidden;text-align:center;font:16px 'Segoe UI',sans-serif;color:#172033}
    .month-value{position:absolute;left:0;right:0;font-size:16px;font-weight:650}.month-cell span{display:block;padding:0 0 12px;font-weight:600}
    #month-timeline button{border:1px solid #cbd5e1;background:white;border-radius:8px;padding:6px 12px;cursor:pointer}
    </style><div id="month-timeline"><p style="font-size:16px">Beide Zeitachsen laufen gemeinsam. Mit Maus, Touch oder Scrollleiste zurückblättern. <button type="button">Zum neuesten Monat →</button></p>''' + ''.join(panels) + '''<p style="font-size:15px">Kontozuwachs = Endstand minus Anfangsstand, einschließlich Erstattungen. Amazon enthält Prime. Die Betragsmaßstäbe sind je Diagramm unterschiedlich.</p></div>
